...
router.get('/walkers/summary', async function(req, res) {
  try {
    req.db.execute(`
      SELECT Users.username AS 'walker_username', COUNT(*) AS 'total_ratings', AVG(WalkRatings.rating) AS average_rating, COUNT(WalkRequests.status = 'completed') FROM WalkRequests INNER JOIN 
    `).then((result) => {
      res.json(result);
    })
  } catch (err) {
